...
async fn main() {
    ...
    let mut prompt =
        cli::read_from_stdin_timeout(Duration::from_millis(config.stdin_read_time)).await;
    ...
    if args.image {
        match send_image_request(1, &args.prompt).await {
            ...
            Err(e) => {
                println!(
                    "Error occurred when requesting image generation from API: {}",
                    e
                )
            }
        }
    // chat request selected
        ...
        if config.open_ai_stream_chat {
            match send_chat_stream_request(
                OpenAiModel::from_str(&config.open_ai_model),
                prompt,
                config.open_ai_max_tokens,
            )
            .await
            {
                Ok(()) => {}
                Err(e) => {
                    println!(
                        "Error occurred when requesting chat response from the API: {}",
                        e
                    )
                }
            }
    ...
    // https://github.com/tokio-rs/tokio/issues/589 reading stdin will block indefinitely
    std::process::exit(0);
}
